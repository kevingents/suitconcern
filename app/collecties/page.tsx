...
export default function CollectiesPage() {
  ...
  return (
    <>
      <PageHero
        eyebrow={t("collectiesHeroEyebrow")}
        title={t("collectiesHeroTitle")}
        description={t("collectiesHeroDescription")}
        crumbs={[{ label: t("home"), href: "/" }, { label: t("collecties") }]}
      />
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-6">
                  <div className="flex items-end justify-between gap-3">
                    <div>
                      <h2 className="font-serif text-2xl text-white">{tcol(`${c.slug}.title`)}</h2>
                      <p className="mt-1 text-sm text-white/70">{tcol(`${c.slug}.description`)}</p>
                      <p className="mt-2 text-xs text-white/50">{c.count} {ts("artikelen")}</p>
                    </div>
